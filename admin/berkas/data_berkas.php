                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example4" class="display min-w850">
                                        ...
                                        <tbody>
                                            <?php
                                            $sql = "SELECT * FROM berkas ORDER BY nik ASC";
                                            $query = mysqli_query($db, $sql);

                                            $no = 1;

                                            while ($brk = mysqli_fetch_array($query)) {
                                            ?>
                                                <tr>
                                                    <td><?php echo $no; ?></td>

                                                    <td><?php echo $brk['nik']; ?></td>
                                                    <td><?php echo $brk['nama_berkas']; ?></td>
                                                    <td><?php echo $brk['keterangan']; ?></td>

                                                    <?php
                                                    $ceksta = $brk['status'];

                                                    if ($ceksta == 'Baru') {
                                                        echo  " <td class=' text-right'><span class='badge badge-danger'>Baru</span></td>";
                                                    } else if ($ceksta == 'Selesai') {
                                                        echo  " <td class=' text-right'><span class='badge badge-success'>Selesai</span>
                                                    </td>";
                                                    }
                                                    ?>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="hapus.php?id_berkas=<?php echo $brk['id_berkas']; ?>" class="btn btn-danger shadow btn-xs sharp" onClick="return confirm('Anda yakin akan menghapus akun <?php echo $brk['nik']; ?> ?');"><i class="fa fa-trash"></i></a>
                                                        <!-- <a href="edit.php?id_penduduk=<?php echo $warga['id_penduduk']; ?>" class="btn btn-warning shadow btn-xs sharp"><i class="fa fa-pencil"></i></a> -->
                                                        <a href="detail.php?id_penduduk=<?php echo $warga['id_penduduk']; ?>" class="btn btn-warning shadow btn-xs sharp"><i class="fa fa-eye"></i></a>
